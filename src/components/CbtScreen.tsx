...
import { 
  ...
  Headphones, CheckCircle2, MessageSquare, Radio, HelpCircle, Award, StopCircle, Upload, Keyboard, AlertCircle, Zap
} from 'lucide-react';
...
export function CbtScreen({ onBack, onNavigate }: CbtScreenProps) {
  ...
  // Modals for 5-minute reset tools
  const [activeModal, setActiveModal] = useState<'reframes' | 'breathe' | 'scribble' | 'shake' | null>(null);
  ...
  // Text-To-Speech helper with speech start/end events
  ...
  return (
    <div className="flex-1 flex flex-col bg-gradient-to-b from-[#f2f8ed] via-[#e8f5e1] to-[#e0f1d6] relative overflow-hidden h-full">
      ...
      {viewMode === 'landing' && (
        <div className="flex-1 flex flex-col justify-between overflow-y-auto">
          {/* Top Bar */}
          <div className="pt-8 pb-1 px-5 flex items-center justify-between">
            <button 
              onClick={onBack} 
            ...
            <button 
              ...
              className="px-4 py-2 rounded-full text-xs font-black flex items-center gap-2 bg-[#a2e372] hover:bg-[#8edb59] text-[#1a3314] shadow-md transition-all active:scale-95"
            >
              <Headphones size={16} className="animate-bounce" />
          ...
          <div className="flex flex-col items-center justify-center my-auto px-4">
            ...
            >
              <Mascot className="w-28 h-28 drop-shadow-sm" />
              <div className="absolute -bottom-2 bg-white/90 text-[#2C3E28] text-[10px] font-black px-2.5 py-1 rounded-full shadow-xs border border-[#d2eaaf] flex items-center gap-1">
            ...
            {/* Title */}
            <h1 className="font-serif italic text-3xl md:text-4xl font-normal text-[#2d4027] tracking-tight text-center mt-6 mb-4">
              What's on your mind?
            </h1>

            {/* Practice Mode Chips */}
            <div className="flex items-center gap-2 mb-4 overflow-x-auto max-w-sm w-full px-1 scrollbar-hide justify-center">
              <button
                onClick={() => setCoachingMode('general')}
                className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
                  coachingMode === 'general' ? 'bg-[#2C3E28] text-[#D5F0C0]' : 'bg-white/80 text-[#5c7a52] border border-[#d2eaaf]'
                }`}
              >
                🌱 Chat
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
                  coachingMode === 'interview' ? 'bg-[#2C3E28] text-[#D5F0C0]' : 'bg-white/80 text-[#5c7a52] border border-[#d2eaaf]'
                }`}
              >
                💼 Mock Interview
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
                  coachingMode === 'presentation' ? 'bg-[#2C3E28] text-[#D5F0C0]' : 'bg-white/80 text-[#5c7a52] border border-[#d2eaaf]'
                }`}
              >
                📊 Presentation
              </button>
            ...
            {/* Prompt Input Card */}
            <div className="w-full max-w-sm bg-white/90 backdrop-blur-xs rounded-[28px] p-5 shadow-sm border border-[#d2eaaf] flex flex-col gap-3">
              <textarea
                value={landingInput}
                ...
                className="w-full bg-transparent resize-none outline-none text-[#2C3E28] placeholder-[#7d9972] text-base font-medium h-20"
              />

              {/* Attachment preview if selected */}
              {attachedFile && renderFileBadge(attachedFile, () => setAttachedFile(null))}
              
              <div className="flex items-center justify-between pt-1 border-t border-[#f0f7e8]">
                {/* Left icons */}
                <div className="flex items-center gap-1">
                  <button 
                    onClick={toggleRecording} 
                    className={`p-2 rounded-full transition-colors ${isRecording ? 'bg-red-100 text-red-600 animate-pulse' : 'text-[#5c7a52] hover:bg-[#f0f9eb]'}`}
                    title="Voice input"
                  >
                    <Mic size={18} />
                  </button>
                  <button 
                    ...
                    className="p-2 text-[#5c7a52] hover:bg-[#f0f9eb] rounded-full transition-colors relative"
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="flex items-center gap-1.5 bg-[#f4fbe8] hover:bg-[#e8f8d6] text-[#3d5932] border border-[#cbe8a8] px-3.5 py-2 rounded-full text-xs font-extrabold transition-all active:scale-95 shadow-2xs"
                  >
                    <Sparkles size={14} className="text-[#64aa38]" />
                    <span>Reframes</span>
                  </button>
                  
                  <button 
                    onClick={() => openChatWithPrompt()}
                    className="flex items-center gap-1.5 bg-[#a2e372] hover:bg-[#8edb59] text-[#1a3314] font-black px-4 py-2 rounded-full text-xs shadow-2xs transition-all active:scale-95"
                  >
                    <span>Chat</span>
                    <Send size={13} />
                  </button>
          ...
          <div className="px-5 mb-6 max-w-sm mx-auto w-full">
            <div className="text-[11px] font-black uppercase text-[#5a7a50] tracking-widest mb-3 px-1">
              5-MINUTE RESET
            </div>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div 
                ...
                className="bg-gradient-to-b from-white to-[#f4faf0] p-4 rounded-3xl border border-[#d8edc4] flex flex-col items-center gap-2 shadow-2xs cursor-pointer active:scale-95 transition-transform"
              >
                <div className="w-12 h-12 rounded-2xl bg-[#e6f7ff] text-[#0092ca] flex items-center justify-center shadow-2xs">
                  <Wind size={22} />
                </div>
                <span className="text-[#21331d] font-extrabold text-sm mt-1">Breathe</span>
                <span className="text-[#7d9972] text-[10px] font-black uppercase">1 MIN</span>
              </div>

              {/* Scribble Card */}
              <div 
                onClick={() => setActiveModal('scribble')}
                className="bg-gradient-to-b from-white to-[#fef8f2] p-4 rounded-3xl border border-[#fce3d1] flex flex-col items-center gap-2 shadow-2xs cursor-pointer active:scale-95 transition-transform"
              >
                <div className="w-12 h-12 rounded-2xl bg-[#fff2e6] text-[#e67e22] flex items-center justify-center shadow-2xs">
                  <Edit3 size={22} />
                </div>
                <span className="text-[#21331d] font-extrabold text-sm mt-1">Scribble</span>
                <span className="text-[#7d9972] text-[10px] font-black uppercase">3 MINS</span>
              </div>
              ...
              <div 
                ...
                className="bg-gradient-to-b from-white to-[#f0faf4] p-4 rounded-3xl border border-[#d3f2e0] flex flex-col items-center gap-2 shadow-2xs cursor-pointer active:scale-95 transition-transform"
              >
                <div className="w-12 h-12 rounded-2xl bg-[#eafaf1] text-[#2ecc71] flex items-center justify-center shadow-2xs">
                  <Activity size={22} />
                </div>
                <span className="text-[#21331d] font-extrabold text-sm mt-1">Shake It</span>
                <span className="text-[#7d9972] text-[10px] font-black uppercase">2 MINS</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Bottom Nav Bar */}
      {viewMode !== 'voice_coach' && (
        <div className="flex justify-around items-center bg-[#f2f8ea] p-3.5 rounded-t-3xl border-t border-[#c2dfa9] text-[#86a674] z-30 mt-auto">
          <button onClick={() => onNavigate?.('home')} className="flex flex-col items-center active:scale-95 transition-transform">
            <Home size={22} />
          ...
          <button 
            onClick={() => setViewMode('landing')}
            className="flex flex-col items-center text-[#4c8435] font-bold"
          >
            <Users size={22} />
            <span className="text-[10px] mt-1 font-extrabold">Socio</span>
          </button>
